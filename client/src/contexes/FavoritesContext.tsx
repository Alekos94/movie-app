...
useEffect(() => {
  async function fetchfavorites () {
    try {
      const response = await fetch('http://localhost:3000/api/movies/favorites')
      if (!response.ok) {
...
async function toggleFavorite (movie: FavoriteMovie) {
  ...
  const updatedFavorites = isFavorite ? favorites.filter(fav => fav.tmdb_id !== movie.tmdb_id) : [...favorites, movie]
